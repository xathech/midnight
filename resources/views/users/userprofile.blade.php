                ...
                <div style="background-color:#29225A;" class="p-4 text-light">
                    ...
                    <form action="{{ route('updateUser') }}" method="post">
                        ...
                        <div class="form-group row">
                            <label for="usernameid" class="col-sm-2 col-form-label">{{__('Username')}}</label>
                            <div class="col-sm-10">
                            <input type="text" class="form-control" id="usernameid" name="username" placeholder="Username" value="{{$currentUser->name}}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="emailid" class="col-sm-2 col-form-label">{{__('E-Mail Address')}}</label>
                            <div class="col-sm-10">
                                <input type="email" class="form-control" id="emailid" name="email" placeholder="Email" value="{{$currentUser->email}}">
                            </div>
                        </div>
                        <div class="row px-3 justify-content-between">
                            <button class="btn btn-outline-danger" type="button" data-toggle="modal" data-target="#modal">{{__('Delete account')}}</button>
                        ...
                        <div class="modal-content" style="background-color:#29225A;">
                            ...
                            <div class="modal-footer justify-content-around">
                                ...
                                <form action="{{ route('deleteUser') }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger">{{__('Yes, I want to delete my account')}}</button>
                                </form>
                            </div>
                        </div>
                        </div>
